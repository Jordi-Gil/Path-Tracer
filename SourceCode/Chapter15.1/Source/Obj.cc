  ...
  while(std::getline(file, line)) {
    ...
    if(par[0] >= '0' and par[1] <= '9') {
      ...
      if(par == "m\\" and cTri == 3) {
        ...
        cTri = 0;
        triangles[count] = Triangle("filename_"+std::to_string(count), tri[0],tri[1],tri[2],(materialB) ? material : mat);
  ...
  while(std::getline(file,line)){
    ...
    if(par == "v") {
      ...
      for(int i = 1; i < face.size()-1; i++){
        aux.push_back(Triangle("filenme_"+std::to_string(i), vertexs[face[0][0]-1],vertexs[face[i][0]-1],vertexs[face[i+1][0]-1],mat));
      }
    }
  }
