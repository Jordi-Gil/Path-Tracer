...
Node *random_scene(int dist) {
        ...
        for(int b = -dist; b < dist; b++){
            ...
            if((center-Vector3(0,0,0)).length() > 0.995){
                if(choose_mat < 0.8){ //diffuse
                    ...
                    list.push_back(new MovingSphere(center, center, 0.0, 1.0, 0.2, new Metal(Vector3(
                    ...
                    list.push_back(new MovingSphere(center,center,0.0,1.0, 0.2, new Dielectric(Vector3::One(),1.5)));
                }
            }
            compare(max, min, list.back()->getCenter());
        }
    ...
    for(int i = 0; i < list.size(); i++) {
        std::cout << list_aux[i]->getCenter() << " || " << list[i]->getCenter() << std::endl;
    }
...
int main(int argc, char **argv) {
    ...
    std::cout << depthTree(world) << std::endl;
