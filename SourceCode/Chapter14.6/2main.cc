...
void iterativeTraversal(Node* root) {
    ...
    while(!stack.empty()) {
        ...
        if(root->obj) {
            root->obj->bounding_box(0,1,root->box); //Leaf node
        }
        else { //Internal node
...
static unsigned int sortedMortonCodes[15] = {4,6,7,18,20,20,20,20,20,20,20,20,20,100,116};
...
int2 determineRange(int size, int idx) {
    ...
    if((idxCode == idxCodeDown) and (idxCode == idxCodeUp)) {
        ...
        while(!dif and idx > 0 and idx < numberObj){
            ...
            if(sortedMortonCodes[idx] != sortedMortonCodes[idx+1]) dif = true;
        }
...
Node* prueba(int size) {
    
    Node* leafNodes = new Node[size];
    Node* internalNodes = new Node[size - 1];
    
    for(int idx = 0; idx < size; idx++){
        std::cout << sortedMortonCodes[idx] + idx << " ";
        sortedMortonCodes[idx] += idx;
        leafNodes[idx].name = "leaf_"+std::to_string(sortedMortonCodes[idx]); 
    }
    std::cout << std::endl;
    std::cout << "--------------------------------" << std::endl;
    ...
    for (int idx = 0; idx < size-1; idx++) {
        ...
        Node *childA;
        if(spliti == first){
            std::cout << "El nodo hoja " << leafNodes[spliti].name << " tiene de padre a " << idx << std::endl;
            childA = &leafNodes[spliti];
        }
        else{ 
            std::cout << "El nodo interno " << spliti << " tiene de padre a " << idx << std::endl;
            childA = &internalNodes[spliti];
        }
        
        Node *childB;
        if(spliti+1 == last) {
            std::cout << "El nodo hoja " << leafNodes[spliti+1].name << " tiene de padre a " << idx << std::endl;
            childB = &leafNodes[spliti+1];
        }
        else{ 
            std::cout << "El nodo interno " << spliti+1 << " tiene de padre a " << idx << std::endl;
            childB = &internalNodes[spliti+1];
        }
        std::cout << std::endl;
...
{
    Node *root = prueba(atoi(argv[1]));
    it(root);
    std::cout << depthTree(root) << std::endl;
    exit(0);    
    if(atoi(argv[1]) == 0){
        Node *root = prueba(atoi(argv[2]));
        it(root);
    }
