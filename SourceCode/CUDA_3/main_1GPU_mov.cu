...
void compare(Vector3 &max, Vector3 &min, Vector3 point) {
 ...
 void create_world(Sphere *h_objects, int &size, int nx, int ny, int dist){
    ...
    for (int a = -dist; a < dist; a++) {
        for (int b = -dist; b < dist; b++) {
            ...
            //if ((center-Vector3(0,0,0)).length() > 0.995) {
                ...
                else if (material < 0.95) h_objects[i] = Sphere(center, 0.2, Material(METAL,Vector3(0.5*(1.0+Random), 0.5*(1.0+Random), 0.5*(1.0+Random)),0.5*Random));
                else h_objects[i] = Sphere(center, 0.2, Material(DIELECTRIC,Vector3::Zero(),-1,1.5));
    ...
    h_objects[0] = Sphere(Vector3(0,-1000,-1), Vector3(0,-1000,-1), 0.0, 1.0, 1000, Material(LAMBERTIAN,Vector3(0.5, 0.5, 0.5))); 
    compare(max,min, h_objects[0].getCenter());
    
    h_objects[1] = Sphere(Vector3( 0, 1, 0), Vector3( 0, 1, 0), 0.0, 1.0, 1.0, Material(DIELECTRIC,Vector3::Zero(),-1,1.5));
    compare(max,min,h_objects[1].getCenter()); //i++;
    
    h_objects[2] = Sphere(Vector3(-4, 1, 0), Vector3(-4, 1, 0), 0.0, 1.0, 1.0, Material(LAMBERTIAN,Vector3(0.4, 0.2, 0.1)));
    compare(max,min,h_objects[2].getCenter()); //i++;
    
    h_objects[3] = Sphere(Vector3( 4, 1, 0), Vector3( 4, 1, 0), 0.0, 1.0, 1.0, Material(METAL,Vector3(0.7, 0.6, 0.5),0.0));
    compare(max,min,h_objects[3].getCenter()); //i++;
    
    h_objects[4] = Sphere(Vector3( 4, 1, 5), Vector3( 4, 1, 5), 0.0, 1.0, 1.0, Material(METAL,Vector3(0.9, 0.2, 0.2),0.0));
...
__global__ void initLeafNodes(Node *leafNodes, int objs, Sphere *d_list) {
    ...
    Material mat = d_list[idx].mat_ptr;
    Vector3 albedo = mat.getAlbedo();
}
...
__global__ void constructBVH(Node *d_internalNodes, Node *leafNodes, int objs, Sphere *d_list) {
    ...
    printf("PARENT %d, LEFT %d_%s, RIGHT %d_%s\n",idx,split,(split==first) ? "l" : "i", split+1, (split+1==last) ? "l" : "i");
}
...
int main(int argc, char **argv) {
  ...
  render_init<<<blocks, nthreads, 0, 0>>>(nx, ny, d_rand_state, seed);
  checkCudaErrors(cudaGetLastError());
  
  initLeafNodes<<<blocks2,threads, 0, 0>>>(d_leafNodes, size, d_objects);
  checkCudaErrors(cudaGetLastError());
  
  constructBVH<<<blocks2,threads, 0, 0>>>(d_internalNodes, d_leafNodes, size-1, d_objects);
  checkCudaErrors(cudaGetLastError());
  
  cudaDeviceSynchronize();
  std::cout << "bound" << std::endl;
  
  boundingBoxBVH<<<blocks2,threads, 0, 0>>>(d_internalNodes, d_leafNodes, size, nodeCounter);
  checkCudaErrors(cudaGetLastError());
  

  cudaDeviceSynchronize();

  render<<<blocks, nthreads, 0, 0>>>(d_frameBuffer, nx, ny, ns, d_cam, d_internalNodes, d_rand_state, depth);
  checkCudaErrors(cudaGetLastError());
