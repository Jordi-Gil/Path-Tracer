...
__global__ void render(Vector3 *fb, int max_x, int max_y, int ns, Camera **cam, Node *world, curandState *d_rand_state, int depth, bool light, bool skybox, Skybox *sky, bool oneTex, unsigned char ** d_textures, int minY, int maxY) {
...
int main(int argc, char **argv) {
  ...
  for(int i = 0; i < numGPUs; i++) {
    ...
    setupCamera<<<1,1>>>(d_cameras[i],nx,ny, scene.getCamera());
    checkCudaErrors(cudaGetLastError());
  ...
  image = "../Resources/Images/GPU_BVH_"+std::to_string(numGPUs)+"_GPU/"+image;
