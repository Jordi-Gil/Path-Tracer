...
__device__ bool Material::scatter(const Ray& r_in, const hit_record &rec, Vector3 &attenuation, Ray& scattered, curandState *random, bool oneTex, unsigned char **d_textures) {
  ...
  if(type == LAMBERTIAN) return Lambertian(rec, attenuation, scattered, random,oneTex, d_textures);
...
__device__ bool Material::Lambertian(const hit_record &rec, Vector3 &attenuation, Ray &scattered, curandState *random, bool oneTex, unsigned char **d_textures) {
