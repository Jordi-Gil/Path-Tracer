...
int main(int argc, char **argv) {
  ...
  if(filter){
    ...
    std::cout << "Filtering image using median filter" << std::endl;
    filenameFiltered = image.substr(0, image.length()-4) + "_median_filter.png";
    
    medianFilter(sx, sy, imageData, imageFiltered);
    stbi_write_png(filenameFiltered.c_str(), sx, sy, 3, imageFiltered, sx*3);
    
    std::cout << "Filtering image using mean filter" << std::endl;
    filenameFiltered = image.substr(0, image.length()-4) + "_mean_filter.png";
    
    meanFilter(sx, sy, imageData, imageFiltered);
    stbi_write_png(filenameFiltered.c_str(), sx, sy, 3, imageFiltered, sx*3);
  }
