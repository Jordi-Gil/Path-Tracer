...
public:
  ...
  Obj loadObj(const std::string &line, bool oneTex, bool &importanceSampling);
  ...
  unsigned int getSizeImpSmp();
  
  Triangle *getObjects();
  Triangle *getImportantSamplings();
  Skybox *getSkybox();
...
private:
  ...
  int nx, ny;
  Triangle *objects;
  Triangle *importanceSamplings;
  Sphere *spheres;
  unsigned int size;
  unsigned int sizeImpSmp;
