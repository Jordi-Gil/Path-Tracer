...
__device__ Vector3 color(const Ray& ray, Node *world, int depth, bool light, bool skybox, curandState *random, Skybox *sky, bool oneTex, unsigned char **d_textures, ShapeList *d_importanceSamplings){
  ...
  for(int i = 0; i < depth; i++){ 
    ...
    if( intersect(world, cur_ray, 0.00001, FLT_MAX, hrec) ) {
      ...
      if(hrec.mat_ptr.scatter(cur_ray, hrec, srec, random, oneTex, d_textures)){
        if(srec.is_specular){ 
...
//           pdf_val = srec.pdf_ptr.value(scattered.direction());
      ...
      if(skybox && sky->hit(cur_ray, 0.00001, FLT_MAX, hrec)){
        return cur_attenuation * hrec.mat_ptr.emitted(hrec.u, hrec.v, oneTex, d_textures);
      }
      else {
        if(light) {
...
__global__ void render(Vector3 *fb, int max_x, int max_y, int ns, Camera **cam, Node *world, curandState *d_rand_state, int depth, bool light, bool skybox, Skybox *sky, bool oneTex, unsigned char ** d_textures, ShapeList *d_imporanceSamplings) {
...
int main(int argc, char **argv) {
  ...
  Triangle *importanceSamplings = scene.getImportantSamplings();
  ...
  if(oneTex){
	...
	size = scene.getSize();
  sizeImpSmp = scene.getSizeImpSmp();
  
  ShapeList *h_importanceSamplings = new ShapeList(importanceSamplings, sizeImpSmp);
  ...
  ShapeList *d_importanceSamplings;
  ...
  cudaMalloc((void **)&d_importanceSamplings, sizeof(ShapeList));
  ...
  h_importanceSamplings->hostToDevice(0);
  ...
  cudaMemcpy(d_importanceSamplings, h_importanceSamplings, sizeof(ShapeList), cudaMemcpyHostToDevice);
  checkCudaErrors(cudaGetLastError());
  ...
  render<<<blocks, nthreads>>>(d_frameBuffer, nx, ny, ns, d_cam, d_internalNodes, d_rand_state, depth, light, skybox, d_skybox, oneTex, d_textures, d_importanceSamplings);
  checkCudaErrors(cudaGetLastError());
  ...
  image = "../Resources/Images/GPU_BVH_OT_PDF_1_GPU/"+image;
