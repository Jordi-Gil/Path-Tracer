...
__global__ void antialiasing(Vector3 *colorResult, curandState local_random, Camera **cam, int max_x, int max_y, int i, int j, Hitable **world, int depth) {
    ...
    colorResult[num] = color(r, world, depth, &local_random);
}
...
__global__ void render(Vector3 *fb, int max_x, int max_y, int ns, Camera **cam, Hitable **world, curandState *d_rand_state, int depth, int minY, int maxY) {
    ...
    size_t size = sizeof(Vector3)*ns;
    Vector3 *colorResult = (Vector3 *) malloc(size);
    //memset(colorResult,0,size);
    
    antialiasing<<<1,8>>>(colorResult, local_random, cam, max_x, max_y, i, j, world, depth);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < numGPUs; i++) {
        ...
        //const size_t malloc_limit = 1024ull * 1024ull * 1024ull;
