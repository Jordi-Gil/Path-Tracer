...
void help(){
  ...
  std::cout << "./path_tracing_NGPUs_dynamic -d"  << std::endl;
  std::cout << "./path_tracing_NGPUs_dynamic -nthreads 16 -sizeX 2000"<< std::endl;
  exit(0);
...
void parse_argv(int argc, char **argv, int &nx, int &ny, int &ns, int &depth, int &dist, int &nthreads, std::string &filename, int &numGPUs,const int count){
  ...
  nx = 600; ny = 200; ns = 100; depth = 50; dist = 11; nthreads = 8; filename = "pic.ppm"; numGPUs = 1;
...
__global__ void antialiasing(Vector3 *colorResult, curandState random, Camera **cam, int max_x, int max_y, int i, int j, Hitable **world, int depth) {
   
	int num = blockIdx.x*blockDim.x + threadIdx.x;
	float u = float(i + curand_uniform(&random))/float(max_x);
	float v = float(j + curand_uniform(&random))/float(max_y);
	Ray r = (*cam) -> get_ray(u,v);
	colorResult[num] = color(r, world, depth, &random);
}
...
__global__ void render(Vector3 *fb, int max_x, int max_y, int ns, Camera **cam, Hitable **world, curandState *d_rand_state, int depth, int minY, int maxY) {
  ...
  curandState random;
	...
	Vector3 *colorResult = (Vector3 *) malloc(sizeof(Vector3)*ns);
	
	antialiasing<<<1,ns>>>(colorResult, random, cam, max_x, max_y, i, j, world, depth);
	//checkCudaErrors(cudaGetLastError());	
	cudaDeviceSynchronize();
	
	Vector3 col(0,0,0);
	
	for (int colRes = 0; colRes < ns; colRes++) {
		col += colorResult[colRes];
	}
...
int main(int argc, char **argv) {
  ...
  std::cout << "Creating " << filename << " with (" << nx << "," << ny << ") pixels with " << nthreads << "x" << nthreads << " using " << numGPUs << " GPUs." << std::endl;
  ...
  int blocks2 = (nx * ny)/(numGPUs * nthreads);
  ...
  std::cout << "GPUs Info - all four GPUs are equals" << std::endl;
  ...
  for (int i = 0; i < numGPUs; i++) {
		cudaSetDevice(i);
          	cudaDeviceSetLimit(cudaLimitDevRuntimePendingLaunchCount, 78732000);		
	...
	for (int i = 0; i < numGPUs; i++) {
		...
		checkCudaErrors(cudaGetLastError());
		render_init<<<blocks, threads>>>(nx, ny, d_randstates[i], seed);
		checkCudaErrors(cudaGetLastError());
		render<<<blocks2, nthreads>>>(d_frames[i], nx, ny, ns, d_cameras[i], d_worlds[i], d_randstates[i], depth, i*(ny/numGPUs), ((i+1)*(ny/numGPUs)));
  ...
  std::cout << "Total Time: " << totalTime << " milisegs."<< std::endl; 
	...
	std::cout << "Printamos el file" << std::endl;
