...
void computeBoundingBox(Node* root) {
    ...
    while(!stack.empty()) {
        ...
        if(root->obj) {
            root->box = (root->obj->tag == SPHERE) ? root->obj->s.getBox() : root->obj->t.getBox(); //Leaf node
        }
        else { //Internal node
...
unsigned int findSplit(Shape *sortedMortonCodes, int first, int last) {
    
  long long firstCode = getMorton(sortedMortonCodes[first]);
  long long lastCode = getMorton(sortedMortonCodes[last]);
  ...
  do {
      ...
      if(newSplit < last){
          ...
          long long splitCode = getMorton(sortedMortonCodes[newSplit]);
...
int2 determineRange(Shape *sortedMortonCodes, int idx, int numberObj) {
    
    if(idx == 0) return int2(0,numberObj);
    
    long long idxCode = getMorton(sortedMortonCodes[idx]);
    long long idxCodeUp = getMorton(sortedMortonCodes[idx+1]);
    long long idxCodeDown = getMorton(sortedMortonCodes[idx-1]);
    ...
    if((idxCode == idxCodeDown) and (idxCode == idxCodeUp)) {
        ...
        while(!dif and idx > 0 and idx < numberObj){
            ...
            if(getMorton(sortedMortonCodes[idx]) != getMorton(sortedMortonCodes[idx+1])) dif = true;
        }
        ...
        do {
            ...
            if(newBoundary >= 0 and newBoundary <= numberObj){
                long long newCode = getMorton(sortedMortonCodes[idx + lmax * d]);
        ...
        for(int t = lmax/2; t >= 1; t /= 2){
            ...
            if(newUpperBound <= numberObj and newUpperBound >= 0){
                long long splitCode = getMorton(sortedMortonCodes[newUpperBound]);
...
Node* generateHierarchy(Shape *sortedMortonCodes, int numberObj) {
    ...
    for(int idx = 0; idx < numberObj; idx++) {
        ...
        if(sortedMortonCodes[idx].tag == SPHERE) 
          sortedMortonCodes[idx].s.setMorton(getMorton(sortedMortonCodes[idx])+idx);
        else sortedMortonCodes[idx].t.setMorton(getMorton(sortedMortonCodes[idx])+idx);
    ...
    for(int idx = 0; idx < numberObj - 1; idx++) {
        ...
        if(idx == 0) std::cout << "idx " << idx << " range [" << range.x << "," << range.y << "] split in " << split << " ";
        ...
        if(idx == 0) std::cout << internalNodes[idx].id << "-->" << " LF: " << internalNodes[idx].left->id << " RG " << internalNodes[idx].right->id << std::endl;
    }
