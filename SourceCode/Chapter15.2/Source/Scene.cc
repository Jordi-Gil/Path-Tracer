...
Triangle loadTriangle(const std::string &line, int counti) {
  ...
  return Triangle("manual_"+std::to_string(counti),position[0],position[1],position[2], mat);
}
...
void Scene::sceneFromFile(const std::string &filename) {
  ...
  while(std::getline(file, line)) {
    ...
    if(aux == "ns") {
      sizes = new int[3]; 
      loadSizes(line, sizes); std::cout << sizes[0] << " " << sizes[1] << " " << sizes[2] << std::endl;
      list_1 = new Shape[sizes[0] + sizes[1]];
      ...
      if(sizes != nullptr and count < (sizes[0] + sizes[1])){
        list_1[count].tag = SPHERE;
        list_1[count].s = loadSphere(line.substr(2,line.size()));
        compare(max, min, list_1[count].s.getCenter());
        ++count;
      }
      ...
      if(sizes != nullptr and count < (sizes[0] + sizes[1])){
        list_1[count].tag = TRIANGLE;
        std::cout << "Loading Triangle " << count << std::endl;
        Triangle tri = loadTriangle(line.substr(2,line.size()),count);
        std::cout << "Triangle loaded" << std::endl;
        list_1[count].t = tri;
        compare(max, min, list_1[count].t.getCentroid());
        ++count;
      }
      else std::cout << "MAX size exceeded" << std::endl;
    }
      ...
      if(sizes != nullptr and num_ob < sizes[2]){
        ...
        std::cout << "OBJ size: " << list_3[num_ob].getSize() << std::endl;
  ...
  int num_objects = count + num_ob_tr;
  
  std::cout << "Real size " << num_objects << std::endl;
  Shape *list_aux = (Shape *) malloc((num_objects) * sizeof(Shape));
  std::copy(list_1, list_1 + count, list_aux);
  //free(list_1);
  ...
  for(int idx = 0; idx < num_objects; idx++) {
    ...
    Vector3 point = (list_aux[idx].tag == SPHERE) ? list_aux[idx].s.getCenter() : list_aux[idx].t.getCentroid();
    
    point[0] = ((point[0] - min_x)/(max_x - min_x));
    point[1] = ((point[1] - min_y)/(max_y - min_y));
    point[2] = ((point[2] - min_z)/(max_z - min_z));
    
    if(list_aux[idx].tag == SPHERE) list_aux[idx].s.setMorton(Helper::morton3D(point[0],point[1],point[2]));
    else list_aux[idx].t.setMorton(Helper::morton3D(point[0],point[1],point[2]));
  }
  ...
  if(num_objects > 0) {
    objects = (Shape *) malloc(num_objects * sizeof(Shape));
...
void Scene::sceneRandom() {
  ...
  list[objs].tag = SPHERE;
  list[objs].s = Sphere(Vector3(0,-1000,0),1000, Material(LAMBERTIAN, Vector3(0.5,0.5,0.5)));
  compare(max, min, list[objs].s.getCenter());
  objs++;
    ...
    for(int b = -dist; b < dist; b++){
      ...
      if((center-Vector3(0,0,0)).length() > 0.995){
        if(choose_mat < 0.8){ //diffuse
          ...
          list[objs].s = Sphere(center, 0.2, Material(LAMBERTIAN, Vector3(
          ...
          list[objs].s = Sphere(center, 0.2, Material(METAL, Vector3(
          ...
          list[objs].s = Sphere(center, 0.2, Material(DIELECTRIC,Vector3::One(),-1.0,1.5));
        }
        else {
          list[objs].s = Sphere(center, 0.2, Material(DIFFUSE_LIGHT, Vector3::One()));
        }
        compare(max, min, list[objs].s.getCenter());
        list[objs].tag = SPHERE;
        objs++;
  ...
  list[objs].tag = SPHERE;
  list[objs].s = Sphere(Vector3(0,1,0), 1.0, Material(DIELECTRIC,Vector3::One(),-1,1.5));
  compare(max, min, list[objs].s.getCenter()); objs++;
  
  list[objs].tag = SPHERE;
  list[objs].s = Sphere(Vector3(-4,1,0),1.0, Material(LAMBERTIAN,Vector3(0.4,0.2,0.1)));
  compare(max, min, list[objs].s.getCenter()); objs++;
  
  list[objs].tag = SPHERE;
  list[objs].s = Sphere(Vector3(4,1,0),1.0, Material(METAL,Vector3(0.7,0.6,0.5),0.0));
  compare(max, min, list[objs].s.getCenter()); objs++;
  
  list[objs].tag = SPHERE;
  list[objs].s = Sphere(Vector3(4,1,5), 1.0, Material(METAL,Vector3(0.9, 0.2, 0.2),0.0));
  compare(max, min, list[objs].s.getCenter()); objs++;
  ...
  for(int idx = 0; idx < objs; idx++) {
    ...
    Vector3 point = list[idx].s.getCenter();
    
    point[0] = ((point[0] - min_x)/(max_x - min_x));
    point[1] = ((point[1] - min_y)/(max_y - min_y));
    point[2] = ((point[2] - min_z)/(max_z - min_z));
    
    list[idx].s.setMorton(Helper::morton3D(point[0],point[1],point[2]));
  }
  
  objects = new Shape[objs];
  std::copy(list, list + objs, objects);
...
void Scene::sceneTriangle() {
  ...
  objects[objs].tag = TRIANGLE;
  objects[objs].t = Triangle("p",Vector3(-4.0,0.0,0.0), Vector3(-2.0,0.0,0.5), Vector3(-3.0,-1.0,0.0) ,Material(LAMBERTIAN, Vector3(1.0,0.0,0.0)));
	compare(max, min, objects[0].t.getCentroid()); objs++;
	
  objects[objs].tag = TRIANGLE;
	objects[objs].t = Triangle("p",Vector3(0.0,0.0,0.0), Vector3(2.0,0.0,0.0), Vector3(1.0,-1.0,0.0) ,Material(METAL, Vector3(1.0,0.0,0.0)));
  compare(max, min, objects[1].t.getCentroid()); objs++;
  ...
  for(int idx = 0; idx < objs; idx++) {
    ...
    Vector3 point = objects[idx].t.getCentroid();
    
    point[0] = ((point[0] - min_x)/(max_x - min_x));
    point[1] = ((point[1] - min_y)/(max_y - min_y));
    point[2] = ((point[2] - min_z)/(max_z - min_z));
  
    objects[idx].t.setMorton(Helper::morton3D(point[0],point[1],point[2])+1);
