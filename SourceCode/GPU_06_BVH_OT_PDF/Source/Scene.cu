...
Obj Scene::loadObj(const std::string &line, bool oneTex, bool &importanceSampling) {
...
void Scene::sceneFromFile(const std::string &filename, const bool oneTex) {
  ...
  while(std::getline(file, line)) {
    ...
    if(aux == "ns") {
      ...
      if(sizes != nullptr and num_ob < sizes[2]){
        list_3[num_ob] = loadObj(line.substr(2,line.size()),oneTex,importanceSampling);
        if(importanceSampling) num_imp_smp += list_3[num_ob].getSize();
  ...
  Triangle *list_aux_imp_smp = new Triangle[num_imp_smp];
  ...
  for(int idx = 0; idx < num_ob; idx++) {
    ...
    if(list_3[idx].isImportant()) std::copy(list_3[idx].getTriangles(), list_3[idx].getTriangles() + n, list_aux_imp_smp + p);
    p += n;
  ...
  if(num_objects > 0) {
    ...
    importanceSamplings = new Triangle[num_imp_smp];
    std::copy(list_aux, list_aux + num_objects, objects);
    std::copy(list_aux_imp_smp, list_aux_imp_smp + num_imp_smp, importanceSamplings);
    std::sort(objects, objects + num_objects, ObjEval());
    size = num_objects;
    sizeImpSmp = num_imp_smp;
  }
}
...
Triangle *Scene::getImportantSamplings() {
  return importanceSamplings;
}
...
unsigned int Scene::getSizeImpSmp() {
  return sizeImpSmp;
}
